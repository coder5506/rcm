...
bool ChessRules::Evaluate() const {
...
bool ChessRules::is_legal() const {
    return Evaluate();
}

bool ChessRules::is_legal(Move move) const {
    const_cast<ChessRules&>(*this).PushMove(move);
    const auto okay = is_legal();
    const_cast<ChessRules&>(*this).PopMove(move);
    return okay;
}

MoveList ChessRules::select_legal(const MoveList& candidates) const {
    MoveList legal;
...
void ChessRules::WhitePawnMoves(vector<Move>& moves, Square square) const {
...
void ChessRules::BlackPawnMoves(vector<Move>& moves, Square square) const {
...
void ChessRules::ShortMoves(
    vector<Move>& moves, Square square, const lte* ptr, SPECIAL special) const
{
    for (lte nbr_moves = *ptr++; nbr_moves != 0; --nbr_moves) {
        const Square dst = static_cast<Square>(*ptr++);
...
void ChessRules::LongMoves(
    vector<Move>& moves, Square square, const lte* ptr) const
{
...
void ChessRules::GenMoveList(vector<Move>& moves) const {
    moves.clear();
...
MoveList ChessRules::GenMoveList() const {
    MoveList candidates;
    GenMoveList(candidates);
    return candidates;
}

MoveList ChessRules::GenLegalMoveList() const {
...
bool ChessRules::Evaluate(vector<Move> *p, TERMINAL& score_terminal) const {
...
Move ChessRules::uci_move(string_view uci_move) const {
...
string ChessRules::move_uci(Move move) const {
    return move.uci();
}

// Read natural string move eg "Nf3"
//  return bool okay
Move ChessRules::san_move(string_view natural_in) const {
    vector<Move> list;
    int  i, len=0;
...
string ChessRules::move_san(Move move) const {
// Improved algorithm
