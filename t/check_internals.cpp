...
TEST_CASE("test castling internals: rook moves") {
    ...
    CHECK(p.d.wking());   CHECK(p.wking_allowed());
    CHECK(p.d.bking());   CHECK(p.bking_allowed());
    CHECK(p.d.wqueen());  CHECK(p.wqueen_allowed());
    CHECK(p.d.bqueen());  CHECK(p.bqueen_allowed());
...
TEST_CASE("test internals") {
    ...
    CHECK(p.d.enpassant_target == SQUARE_INVALID);
    CHECK(p.d.wking_square == e1);
    CHECK(p.d.bking_square == e8);
    CHECK(p.d.wking());  CHECK(p.d.wqueen());
    CHECK(p.d.bking());  CHECK(!p.d.bqueen());
